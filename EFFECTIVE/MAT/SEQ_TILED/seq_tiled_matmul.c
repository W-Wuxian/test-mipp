...
int main(int argc, char** argv) {
    if (argc != 6) {
        printf("Usage: %s <ni> <nj> <nk> <tl> <s>\n", argv[0]);
        return 1;
    }

    //srand(1234567);
    char* optim = "seq+tiled";
    int ni = atoi(argv[1]);
    int nj = atoi(argv[2]);
    int nk = atoi(argv[3]);
    ...
    for (int i = 0; i < sample; i++){
        start = clock();
        seq_tiled_matmul(tile, ni, nj, nk, A, B, C, D);
        end = clock();
        cpu_time_used += ((double) (end-start)) / CLOCKS_PER_SEC;
        test = checksum(ni, nj, D);
        printf("MAX_INT %d iter %d checksum %lf Time %lf TotalTime %lf\n", INT_MAX, i, test, cpu_time_used/(i+1), cpu_time_used);
        memset(D, 0, sizeof(REAL) * ni * nk);
    }

    FILE* fptr =  fopen("MATMUL_BENCH_SEQ_TILED.csv", "a+");
    if (fptr == NULL) {
        free(A);
        A = NULL;
        free(B);
